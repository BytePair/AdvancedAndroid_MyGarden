...
public class PlantWidgetProvider extends AppWidgetProvider {
    ...
    static void updateAppWidget(Context context, AppWidgetManager appWidgetManager,
        ...
        // COMPLETE (4): separate the updateAppWidget logic into getGardenGridRemoteView and getSinglePlantRemoteView
        // COMPLETE (5): Use getAppWidgetOptions to get widget width and use the appropriate RemoteView method
    ...
    private static RemoteViews getSinglePlantRemoteView(Context context, int imgRes, long plantId, boolean showWater) {
        ...
        // Return views when finished
        return views;
    }

    private static RemoteViews getGardenGridRemoteView(Context context) {
        // COMPLETE (6): Set the PendingIntent template in getGardenGridRemoteView to launch PlantDetailActivity
        RemoteViews remoteViews = new RemoteViews(context.getPackageName(), R.layout.widget_grid_view);

        // set the GridRemoteViewsService intent to act as the adapter for the GridView
        Intent intent = new Intent(context, GridRemoveViewsService.class);
        remoteViews.setRemoteAdapter(R.id.widget_plant_grid_view, intent);

        // set the PlantDetailActivity intent to launch when clicked
        Intent openAppIntent = new Intent(context, PlantDetailActivity.class);
        PendingIntent openAppPendingIntent = PendingIntent.getActivity(context, 0, openAppIntent, PendingIntent.FLAG_UPDATE_CURRENT);
        remoteViews.setPendingIntentTemplate(R.id.widget_plant_grid_view, openAppPendingIntent);

        // handle empty gardens
        remoteViews.setEmptyView(R.id.widget_plant_grid_view, R.id.empty_relative_layout);
